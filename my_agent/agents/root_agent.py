...
def create_root_agent(a2a_tools: dict) -> Agent:
    ...
    return Agent(
        ...
        instruction=(
            ...
            "3. For combined queries (time + weather), call BOTH tools and combine the results yourself:\n"
            "   Example: '런던 시간과 날씨' → call send_to_time_agent AND send_to_weather_agent, then synthesize\n\n"
            "4. Default city is 'Seoul' if not specified by user\n"
            "5. Always pass user_id and session_id from the conversation context\n"
            "6. Respond naturally in Korean, presenting agent responses to the user\n\n"
            "Examples:\n"
            "- User: '서울 날씨' → Use send_to_weather_agent('Seoul', user_id, session_id)\n"
            "- User: '도쿄 시간' → Use send_to_time_agent('Tokyo', user_id, session_id)\n"
            "- User: '런던 시간과 날씨' → Use BOTH send_to_time_agent('London', ...) AND send_to_weather_agent('London', ...), then combine\n"
